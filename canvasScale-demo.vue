        ...
        <div class="canvas_container">
            ...
            <div class="canvas_main" @scroll.passive="getScroll($event)">
                ...
                <div :style="'width:' + width + 'px;height:' + height + 'px;overflow: hidden;'">
                    <!--背景-->
                    <canvas :id="radom3" class="background_canvas" :width="width" :height="height"></canvas>
                    <!--学生-->
...
<script>
	...
	export default {
		...
		watch: {
			...
			wsImageURL: function () {
				if (this.alertPage === this.pageCount.indexOf(this.alertItem.wsPage)) {
					this.studentCanvas();
				}
			},
			alertPage: function () {
		...
		methods: {
			// 画背景
			backgroundCanvas (str) {
				let _this = this;
				const canvas3 = document.getElementById(this.radom3);
				this.context3 = canvas3.getContext('2d');
				let bgImgCanvas = new Image();
				bgImgCanvas.setAttribute('crossOrigin', 'Anonymous');
				let cacheIdx = _this.pageCount.indexOf(_this.alertItem.wsPage);
				bgImgCanvas.src = _this.alertItem.openStudentSubject[cacheIdx].backgroundImageHttp;
				_this.context3.clearRect(0, 0, this.width, this.height);
				bgImgCanvas.onload = function () {
					_this.context3.drawImage(bgImgCanvas, 0, 0, _this.width, _this.height);
				}
			},
			// 画学生笔迹
			studentCanvas () {
				...
				let studentImgCanvas = new Image();
				studentImgCanvas.setAttribute('crossOrigin', 'Anonymous');
				let cacheIdx = _this.pageCount.indexOf(_this.alertItem.wsPage);
				studentImgCanvas.src = _this.alertItem.openStudentSubject[cacheIdx].studentImageUrl;
				_this.context2.clearRect(0, 0, _this.width, _this.height);
				// _this.context2.createImageData(_this.width,_this.height);
				studentImgCanvas.onload = function () {
					_this.context2.drawImage(studentImgCanvas, 0, 0, _this.width, _this.height);
				}
			},
			initDraw (imgUrl) {
				let _this = this;
				...
				if (imgUrl) {
					...
					let cacheIdx = _this.pageCount.indexOf(_this.alertItem.wsPage);
					_this.img.src = _this.alertItem.openStudentSubject[cacheIdx].teacherImageUrl;
				}
				//  加载图片
				_this.context1.clearRect(0, 0, this.width, this.height);
				_this.img.onload = function(){
					// _this.context1.createImageData(_this.width,_this.height);
					_this.context1.drawImage(_this.img, 0, 0, _this.width, _this.height);
					// _this.context1.drawImage(_this.img, 0, 0, _this.width, _this.height,_this.imgX, _this.imgY, _this.width * _this.scale, _this.height * _this.scale);
				}
			},
			...
			canvasDown (e, type) {
				if (this.canDraw) {
					...
					let canvasX, canvasY;
					if ('touch' === type) {
						canvasX = e.targetTouches[0].pageX - document.getElementById(this.radom1).getBoundingClientRect().left;
						canvasY = e.targetTouches[0].pageY - document.getElementById(this.radom1).getBoundingClientRect().top;
					} else {
						canvasX = e.offsetX;
						canvasY = e.offsetY;
                    }
			...
			canvasMove (e, type) {
				...
				if (undefined != e.touches && 1 < e.touches.length && chmapScale < 3.1) {
					...
					// let posTouch = _this.windowToCanvas (nowTouch[0].clientX, nowTouch[0].clientY);
					// let posObj = {x:((posTouch.x-_this.imgX)/_this.scale).toFixed(5) , y:((posTouch.y-_this.imgY)/_this.scale).toFixed(5)};
			...
			getBackgroundImgData() {
				let canvas = document.getElementById(this.radom3);
				let base64String = canvas.toDataURL('image/png');
				return base64String;
			},
			...
			magnify(num) {
				if(this.scale < 3.5) {
					if(this.canvasUrl1 == null) {
						...
						let canvas3 = document.getElementById(this.radom3);
						this.canvasUrl1 = canvas.toDataURL('image/png');
						this.canvasUrl2 = canvas2.toDataURL('image/png');
						this.canvasUrl3 = canvas3.toDataURL('image/png');
					}
					if (num) {
